...
package com.muzi.frescoexample.zoomable;
...
public class ZoomableDraweeView extends DraweeView<GenericDraweeHierarchy> implements ScrollingView {
    ...
    public void rotateRight() {
        openRotate(RotationOptions.ROTATE_90);
    }
    ...
    private float curreRotate;

    public void openRotate(float rotateAngle) {
        curreRotate = getRotation();
        if (animator == null) {
            animator = ValueAnimator.ofFloat(curreRotate, (curreRotate + rotateAngle));
            animator.setInterpolator(new LinearInterpolator());
            animator.setDuration(500);
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    ZoomableDraweeView.this.setRotation((Float) animation.getAnimatedValue());
                }
            });
            ...
            animator.setFloatValues(curreRotate, (curreRotate + rotateAngle));
        }
        animator.start();
    }
